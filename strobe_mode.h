void strobe_mode(LEDStruct& leds, bool mc) {
	...
	switch (leds.led_mode) { // keep combo_modes array updated to include led_mode values that use a combo LEDStruct (combo[A,B,C,D]_leds)
		...
		case 5:
			if (mc) { leds.this_delay = 10; leds.this_speed = 12; leds.that_speed = 6; leds.this_rot = 1; leds.that_rot = 10; leds.all_freq = 3; leds.this_cutoff = 200; leds.that_cutoff = 128;  leds.this_sat = 255; that_sat = 255; }
			two_sin(leds);
			break;
		
		case 6:
			if (mc) { leds.this_delay = 15; leds.this_speed = 12; leds.that_speed = 6; leds.this_rot = 1; leds.that_rot = 10; leds.all_freq = 3; leds.this_cutoff = 200; leds.that_cutoff = 100; leds.this_sat = 255; that_sat = 255; }
			two_sin_ring(leds);
			break;

		case 7:
			if (mc) { leds.this_delay = 15; leds.target_palette = stratosphere_sunset_gp;  leds.this_speed = 12; leds.that_speed = 6; leds.this_rot = 1; leds.that_rot = 10; leds.all_freq = 3; leds.this_cutoff = 200; leds.that_cutoff = 100; leds.this_sat = 255; that_sat = 255; }
		...
		case 18:
			if (mc) { leds.this_delay = 10; leds.target_palette = scoutie_gp; leds.this_fade = 200; leds.numdots = 3; leds.ringBeat = { 5, 10, 3, 1 }; leds.this_bright = 255; leds.this_diff = 6; leds.juggle_index_reset = 0; }
			juggle_columns_all(leds);
			break;

		case 19:
			if (mc) { leds.this_delay = 10; leds.target_palette = cequal_gp; leds.this_fade = 164; leds.numdots = 4; leds.ringBeat = { 5, 1, 3, 1 }; leds.this_bright = 255; leds.this_diff = 6; leds.juggle_index_reset = 0; }
		...
		case 25:
			if (mc) { leds.this_delay = 10; leds.sparking_columns = { 50, 60, 20, 80 }; leds.cooling_columns = { 50, 40, 30, 60 }; }
			fire_columns(leds);
			break;

		case 26:
			if (mc) { leds.this_delay = 10; leds.target_palette = slope_gp; leds.sparking_columns = { 50, 60, 20, 80 }; leds.cooling_columns = { 50, 40, 30, 60 }; }
		...
		case 28:
			if (mc) { leds.this_delay = 10; leds.target_palette = Fire_1_gp; leds.sparking_columns = { 50, 60, 20, 80 }; leds.cooling_columns = { 50, 40, 30, 60 }; }
