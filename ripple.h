...
void ripple_through_pal(LEDStruct& leds) {
	fadeToBlackBy(leds.strip, NUM_LEDS, leds.this_fade);
	...
	default:
		ringPaletteAdd(leds, (leds.ripple_center + leds.ripple_step + STRIP_LENGTH) % STRIP_LENGTH, leds.current_palette, leds.ripple_index + leds.ripple_step, leds.ripple_fade / leds.ripple_step * 2, leds.current_blending);
		ringPaletteAdd(leds, (leds.ripple_center - leds.ripple_step + STRIP_LENGTH) % STRIP_LENGTH, leds.current_palette, leds.ripple_index - leds.ripple_step, leds.ripple_fade / leds.ripple_step * 2, leds.current_blending);
		leds.ripple_step++;
		break;
	}
