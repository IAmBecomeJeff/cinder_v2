	...
	while (Serial.available() > 0) {
		...
		switch (in_byte) {
			...
			case 118:
				...
				// Command: w Change cooling (0-255)
			case 119:
				this_arg = Serial.parseInt();
				...
				// Command: x Change sparking (0-255)
			case 120:
				this_arg = Serial.parseInt();
				...
				// Command: y Change circ_scale (0-255)
			case 121:
				this_arg = Serial.parseInt();
				...
				// Command: z Choose transition (0 - max_transition)
			case 122:
				if (!transition_lock) {
					transition_lock = 1;
				}
				this_arg = Serial.parseInt();
				transitioning = constrain(this_arg, 0, max_transitions);
				break;

				// command: Z - toggle transition_lock
			case 90:
				this_arg = Serial.parseInt();
				transition_lock = !transition_lock;
				Serial.print("transition lock is: ");
				if (transition_lock) { Serial.println("on"); }
				else { Serial.println("off"); }
				break;

				// command: B - change palette of comboB (for fire)
			case 66:
				this_arg = Serial.parseInt();
				comboB_leds.palette_index = this_arg % palette_count;
				comboB_leds.target_palette = palette_array[comboB_leds.palette_index];
				Serial.print("ComboB Palette: ");
				Serial.println(comboB_leds.palette_index);
				break;

				// command: D - change palette of comboD (for fire)
			case 68:
				this_arg = Serial.parseInt();
				comboD_leds.palette_index = this_arg % palette_count;
				comboD_leds.target_palette = palette_array[comboD_leds.palette_index];
				Serial.print("ComboD Palette: ");
				Serial.println(comboD_leds.palette_index);
				break;
